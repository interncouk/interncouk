...
/*!****************************************************************!*\
  !*** ./platform/plugins/job-board/resources/assets/js/main.js ***!
  \****************************************************************/
...
!function ($) {
  $(function () {
    $('.menu-item.has-submenu .menu-link').on('click', function (e) {
      e.preventDefault();
      $(this).next('.submenu').is(':hidden') && $(this).parent('.has-submenu').siblings().find('.submenu').slideUp(200);
    ...
    $('.btn-aside-minimize').on('click', function () {
      window.innerWidth < 768 ? (e('body').removeClass('aside-mini'), $('.screen-overlay').removeClass('show'), $('.navbar-aside').removeClass('show'), $('body').removeClass('offcanvas-active')) : $('body').toggleClass('aside-mini');
    });
    $('.select-nice').length && $('.select-nice').select2();
    if ($('#offcanvas_aside').length) {
      var _e = document.querySelector('#offcanvas_aside');
      new PerfectScrollbar(_e);
    }
    ...
    if (window.noticeMessages && window.noticeMessages.length) {
      window.noticeMessages.map(function (x, k) {
