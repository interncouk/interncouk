    ...
    public function run(): void
    {
        ...
        foreach ($data as $locale => $options) {
            $options = collect($options)
                ->mapWithKeys(function (string $value, string $key) use ($settingPrefix, $locale) {
                    return [$settingPrefix . ($locale != 'en_US' ? $locale . '-' : '') . $key => $value];
                })
                ->all();
        ...
        Setting::set($settingPrefix . 'vi-social_links', json_encode($socialLinks));
